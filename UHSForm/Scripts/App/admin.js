...
app.service("crudCustomerService", ["$http", function ($http) {
    ...
    this.GetUpdateCustomerDetails = function (cuID) {
        return $http({
            method: "GET",
            url: "/Admin/Accounts/GetUpdateCustomerDetails",
            params: { cuID: cuID },
            headers: { "content-type": "application/json" },
        }).then(function (response) {
            return response.data;
    ...
    this.GetPackagesByServices = function (catID, catsubID, proprestID) {
        return $http({
            method: "GET",
            url: "/Admin/Packages/GetPackagesByServicesWithOutProperty",
            params: {
                catID: catID, catsubID: catsubID, proprestID: proprestID},
            headers: { "content-type": "application/json" },
        }).then(function (response) {
            return response.data;
        });
...
app.service("crudPropService", ["$http", function ($http) {
    ...
    this.GetPropertyDropDownByAreasID = function (propaID, subAreaID) {
        return $http({
            method: "GET",
            url: "/Admin/Project/GetPropertyDropDownByAreasID",
...
app.controller("BookingController", function ($http, $filter, $scope, crudPropService, crudDropdownServices, DTOptionsBuilder, $timeout, CRUDDashboardServices, LogoutServices, $window, crudUserService, crudCustomerService, DTOptionsBuilder) {
    var Auth = LogoutServices.getValue();
    if (Auth == null || Auth == false) {
        $window.location.href = "/Account/Index";
    }
        ...
        $scope.PropertyAddDisable = true;
        $scope.ResidentialAddDisable = true;
        $scope.SubCategoryDropdown = [
            { ID: 1, Value: 'Regular Cleaning' },
            { ID: 2, Value: 'Deep Cleaning' },
            { ID: 3, Value: 'Specialized Cleaning' },
        ];
        $scope.PaymentStatusDropdown = [
            { ID: 1, Value: 'Paid' },
            { ID: 2, Value: 'Deep Cleaning' },
            { ID: 3, Value: 'Specialized Cleaning' },
        ];
        $scope.ChangeArea = function () {
            console.log($scope.ddlArea);
        ...
        $scope.GetPropbySubArea = function () {
            ...
            console.log("Test");
            crudPropService.GetPropertyDropDownByAreasID($scope.ddlArea, $scope.ddlsubArea).then(function (result) {
                console.log(result);
                if (result == "Exception") {
                }
                else {
                    $scope.PropertyAddDisable = false;
                    $scope.PropertyDropdown = result;
        ...
        $scope.GetPropDetails = function () {
            $scope.ResidentialAddDisable = false;
        }
...
app.controller("ServiceBookingController", function ($http, $filter, $scope, crudPropService, crudDropdownServices, DTOptionsBuilder, $timeout, CRUDDashboardServices, LogoutServices, $window, crudUserService, crudCustomerService, DTOptionsBuilder) {

    //crudCustomerService.GetUpdateCustomerDetails(713).then(function (result) {
    //    console.log(result);
    //    if (result == "Exception") {
    //    }
    //    else if (result.length !== 0) {
    ...
    //        console.log(result);
    //        crudCustomerService.GetPackagesByServices(1, 1, $scope.AdditionalDetails[0].proprestID).then(function (result) {
    ...
    $scope.GetCustomerDetails = function () {
        crudCustomerService.GetUpdateCustomerDetails($scope.ddlcustomerID).then(function (result) {
            console.log(result);
            if (result == "Exception") {
            }
            else if (result.length !== 0) {
    ...
    $scope.selectFrequency = function (value) {
        ...
        $scope.PackageName = value.PackageName;
        ...
        $scope.serviceOptions = [
            ...
            { value: "1", text: `3 Months (${servicesPerWeek * 12} service)` },
            { value: "2", text: `6 Months (${servicesPerWeek * 24} service)` },
            { value: "3", text: `12 Months (${servicesPerWeek * 48} service)` }
        ];

        if (value.PackageName == 'One Time Service') {
           
            $scope.rows = new Array(1);

            $scope.MultipleTime = true;
        }
        else {
        ...
        const duration = Number($scope.DurationH); // In minutes
        $scope.timeOptions = generateTimeOptions("08:00", "18:00", 15); // 15-minute intervals
        console.log($scope.timeOptions);
        $scope.endTimeOptions = $scope.timeOptions.slice(); // Initialize with all time options
        /*$scope.timeSlots = generateTimeRanges(startTime, endTime, duration, $scope.measurementH);*/
    }

    $scope.onMonthSelection = function () {
        // Calculate the number of rows based on ddlNoOfMonths and ServicePack
        if ($scope.ddlNoOfMonths == 1) {
            $scope.numRows = $scope.ServicePack * 12;
        } else if ($scope.ddlNoOfMonths == 2) {
            $scope.numRows = $scope.ServicePack * 24;
        } else if ($scope.ddlNoOfMonths == 3) {
            $scope.numRows = $scope.ServicePack * 48;
        } else if ($scope.ddlNoOfMonths == 4) {
            $scope.numRows = $scope.ServicePack * 4;
        } else {
            $scope.numRows = 0;
        }
        console.log($scope.numRows);
        $scope.rows = new Array($scope.numRows);
    ...
    $scope.initFlatpickr = function (index) {
        // Initialize Flatpickr for each row's start date field
        $timeout(function () {
            flatpickr('#kt_speciClenDate' + index, {
                inline: false,
                enableTime: false,
    ...
    $scope.initSelect2 = function (index) {
        // Initialize Select2 for each row's time select field
        $timeout(function () {
            $('#DTimeID' + (index + 1)).select2({
                placeholder: "Select a Time...",
                dropdownParent: $("#kt_UpdateDetails")  // Adjust this selector as necessary
            });
        });
    };

    $scope.initSelectEnd2 = function (index) {
        // Initialize Select2 for each row's time select field
        $timeout(function () {
            $('#EDTimeID' + (index + 1)).select2({
                placeholder: "Select a Time...",
                dropdownParent: $("#kt_UpdateDetails")  // Adjust this selector as necessary
            });
        });
   ...
   // Update end time options based on selected start time
    $scope.updateEndTRTimeOptions = function () {
        ...
        const startIndex = $scope.timeOptions.indexOf($scope.selectedTimes);
    ...
    function generateTimeOptions(start, end, interval) {
        ...
        while (startTime <= endTime) { // Include the end time by using <=
    ...
    function formatTime(date) {
        ...
        return `${formattedHours}:${formattedMinutes} ${period}`; // Added space between minutes and period
    }
    ...
    function generateTimeRanges(start, end, duration, unit) {
        // Convert duration to minutes if needed
        const durationInMinutes = unit === "Hour" ? duration * 60 : duration;

        const ranges = [];
        let current = moment(start, "hh:mm A");
        const endMoment = moment(end, "hh:mm A");

        while (current.isBefore(endMoment)) {
            const rangeStart = current.format("hh:mm A");
            const rangeEnd = current.clone().add(durationInMinutes, 'minutes').format("hh:mm A");

            if (moment(rangeEnd, "hh:mm A").isAfter(endMoment)) {
                break; // Stop if the range exceeds the end time
            }

            ranges.push(`${rangeStart} ${rangeEnd}`);
            current.add(durationInMinutes, 'minutes'); // Move to the next range start
        }
        return ranges;
    }
...
app.controller("CustomerController", function ($http, $filter, $scope, crudDropdownServices, DTOptionsBuilder, $timeout, CRUDDashboardServices, LogoutServices, $window, crudUserService, crudCustomerService, DTOptionsBuilder) {
    ...
    if (Auth == null || Auth == false) {
        ...
        $scope.InitAddCustomer = function () {
            // Clear the select2 selection
            $scope.txtTeam = null;
            // Get the select2 instance
            var $selectTeam = $("#TeamAssign");
            // Clear the select2 selection
            $selectTeam.val(null).trigger("change.select2");
            $scope.AddCustomerForm.$setPristine(); // Reset form
            $scope.AddCustomerForm.$setUntouched(); // Reset form
        }
