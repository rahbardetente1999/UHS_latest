...
namespace UHSForm.Areas.Staff.Controllers
{
    ...
    public class MyProfileController : Controller
    {
        ...
        public ActionResult GetUpdateUserDetails()
        {
            ...
            {
                var objUser = _objGeneralDB.GetUserLoginDetails(User.Identity.Name);
                var result = _objGeneralDB.GetUpdateUserDetails(objUser.uID, null, objUser.stfID, objUser.rID, null);
                return Json(result, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult UploadProfilePic()
        {
            ...
            try
            {
                ...
                if (fileCount > 0)
                {
                    ...
                    int CountProfileFile = UhDb.Files.Where(x => x.stfID == ID && x.FileUse == 2 && x.IsActive == true && x.IsDelete == false).Count();
                    ...
                    {
                        var objFiles = UhDb.Files.Where(x => x.stfID == ID && x.FileUse == 2 && x.IsActive == true && x.IsDelete == false).FirstOrDefault();
                        objFiles.Filename = Path.GetFileName(postedFile.FileName);
                        objFiles.FileSize = postedFile.ContentLength.ToString();
                        objFiles.FileContentType = "Image";
                        objFiles.FileFieldName = path;
